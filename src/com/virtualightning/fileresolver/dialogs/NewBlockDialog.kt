...
class NewBlockDialog(baseUI : BaseUI, callback : ICallback<Block>) : BaseDialog<Block>(builder,baseUI,callback) {
    init {
        ...
        var tempLabel = JLabel("Block Name : ")
        constraint.gridx = 0
        constraint.gridy = 0
        ...
        confirmBtn.addActionListener Listener@{
            val blockName = input.text

            if(!blockName.matches(Regex("^[a-zA-Z0-9]{1,11}$"))) {
                tips.text = "Block name must be a string consisting of numbers,letters,and the length less than 11 "
                return@Listener
            }

            setResultAndQuit(true,Block(blockName))
        }
